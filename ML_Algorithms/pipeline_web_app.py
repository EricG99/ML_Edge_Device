...
def retraining_thread_task(retraining_data_df: pd.DataFrame, algorithm: str):
    ...
    try:
        ...
        if algorithm == 'lstm':
            ...
            with shared_resource_lock:
                shared_model.update({
                    "model": new_model, "scaler": new_scaler, "features": new_features,
...
def inference_manager(config: dict, inference_class, folder_flag: str, algorithm: str, mode: str):
    ...
    try:
        ...
        for cycle in range(max_cycles):
            ...
            # Der Iterator wird hier für den aktuellen Zyklus aufgerufen
            ...
            if mode == "retraining" and cycle < max_cycles - 1:
                ...
                logging.info("--- Inferenz-Manager pausiert und wartet auf Abschluss des Retrainings... ---")
                retraining_thread.join()
                logging.info("--- Retraining abgeschlossen. Inferenz-Manager wird fortgesetzt. ---")
        ...
        if all_predictions:
            try:
                ...
                logging.error(f"Fehler beim finalen SpeichAern der Ergebnisse: {e}", exc_info=True)
