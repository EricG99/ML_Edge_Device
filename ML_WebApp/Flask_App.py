...
def run_flask_server(host='0.0.0.0', port=5001, debug=False, use_reloader=False):
    ...
    if app:
        try:
            # Versuche die lokale IP-Adresse zu bestimmen
            hostname = socket.gethostname()
            local_ip = socket.gethostbyname(hostname)
            logging.info(f"Flask-Server startet auf http://{host}:{port} (Geräte-IP: {local_ip})")
        except socket.gaierror:
            logging.warning("Konnte lokale IP-Adresse nicht bestimmen. Starte Flask-Server ohne IP-Anzeige.")
